...
function parseGeoJsonAssets(payload: unknown): CompanyAsset[] {
...
export default function CompanyAssetMode({ candidates, scannedLakes, onSelectCandidate }: CompanyAssetModeProps) {
  ...
  const importGeoJson = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        setMessage(`Импортировано объектов: ${imported.length}. Они не отправлялись на сервер.`);
      } catch (cause) {
