...
function caseHref(candidate: RegionalObservationScan["candidates"][number], mlCaseId?: string | null) {
  const href = riskTwinHref({
  ...
  return mlCaseId ? `${href}&ml_case=${encodeURIComponent(mlCaseId)}` : href;
}

function decisionTitleRu(decision: IntegratedRiskTwinCase["decision"] | undefined) {
  if (!decision) return null;
  return {
    missing_ml_evidence: "Запустить ML-анализ связанного ледника",
    ml_boundary_review: "Сначала проверить ML-границу ледника",
    lake_boundary_review: "Следующее действие: проверить контур воды",
    route_linkage_review: "Подтвердить гидрологическую связь",
  }[decision.driver];
}

function decisionActionRu(decision: IntegratedRiskTwinCase["decision"] | undefined) {
  if (!decision) return null;
  return {
    missing_ml_evidence: "Построить glacier-specific boundary, probability и entropy для связанного ледника перед интерпретацией его текущей геометрии.",
    ml_boundary_review: "Сверить raw и inventory-guided ML-контуры с исходной сценой в полном разрешении; не использовать изменение площади, пока расхождение не объяснено.",
    lake_boundary_review: "Получить чистую сцену и подтвердить инвентарный контур озера перед полевой проверкой.",
    route_linkage_review: "Сверить HydroRIVERS planning-route с инженерной схемой оператора и локальной гидравликой.",
  }[decision.driver];
}
...
export default function CommandCenter() {
  ...
  const [integratedCase, setIntegratedCase] = useState<IntegratedRiskTwinCase | null>(null);
  const [contextStatus, setContextStatus] = useState<"idle" | "loading" | "ready" | "unavailable">("idle");
  const [forceMlRun, setForceMlRun] = useState(false);
  ...
  useEffect(() => {
    ...
    let active = true;
    setContext(null);
    setIntegratedCase(null);
    setContextStatus("loading");
    setSelectedObjectId(selectedCase.lake_id ? `lake:${selectedCase.lake_id}` : `glacier:${selectedGlacier.rgi_id}`);
    fetchIntegratedRiskTwinCase(selectedGlacier.rgi_id, {
      year: 2024,
      lakeInventoryYear: selectedCase.inventory_year,
      lakeId: selectedCase.lake_id,
      bufferKm: 10,
      runMlIfMissing: forceMlRun || companyAsset?.isPublicExample === true,
    })
      .then((next) => {
        if (!active) return;
        setIntegratedCase(next);
        setContext(next.context);
        setContextStatus("ready");
      })
      .catch((cause) => {
        if (!active) return;
        // Keep a rolling local deployment useful if the API has not restarted
        // onto the integrated endpoint yet. This fallback never invents ML.
        fetchRiskTwinContext(selectedGlacier.rgi_id, 2024, 10, selectedCase.inventory_year)
          .then((next) => {
            if (!active) return;
            setContext(next);
            setContextStatus("ready");
            setError("ML-интеграция API пока недоступна; показан GIS-only контекст. Перезапустите API.");
          })
          .catch(() => {
            if (!active) return;
            setContextStatus("unavailable");
            setError(cause instanceof Error ? cause.message : "Не удалось загрузить интегрированный контекст выбранного объекта.");
          });
      });
    return () => { active = false; };
  }, [companyAsset?.isPublicExample, forceMlRun, selectedCase, selectedGlacier]);
  ...
  const mlEvidence = integratedCase?.ml_evidence ?? null;
  const workflowPriority = integratedCase?.decision.workflow_priority_0_100 ?? selectedCase?.observation_priority_0_100 ?? 0;
  const integratedDecisionTitle = decisionTitleRu(integratedCase?.decision);
  const integratedDecisionAction = decisionActionRu(integratedCase?.decision);
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-6 sm:py-9">
        ...
        {!selectedCase || !selectedGlacier ? <section className="rounded-3xl bg-white p-6 shadow-sm"><p className="text-sm text-slate-600">Загружаем первый объект из очереди…</p></section> : <>
          <section aria-labelledby="answer-title" className="rounded-3xl border border-cyan-200 bg-white p-5 shadow-sm sm:p-7">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-cyan-800">Ответ системы</p><h2 id="answer-title" className="mt-1 text-2xl font-bold tracking-tight text-slate-950">{integratedDecisionTitle ?? `Начать с объекта №${selectedCaseIndex || 1}`}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Ледниковое озеро в координатах {coordinate(selectedCase.latitude)}° N · {coordinate(selectedCase.longitude)}° E. Теперь решение объединяет инвентарь озера, glacier-specific ML, RGI и downstream‑контекст; это всё ещё не вероятность события.</p></div><div className="rounded-2xl bg-slate-950 px-5 py-4 text-center text-white"><p className="text-xs font-bold uppercase tracking-wide text-cyan-200">Приоритет следующей задачи</p><p className="mt-1 text-4xl font-black">{workflowPriority.toFixed(0)}<span className="text-lg text-slate-300">/100</span></p>{integratedCase && <p className="mt-1 text-[11px] text-slate-300">озеро {integratedCase.decision.lake_observation_priority_0_100} · ML {integratedCase.decision.ml_boundary_review_priority_0_100 ?? "нет"}</p>}</div></div>
          ...
          <section aria-labelledby="ml-decision-title" className={`rounded-3xl border p-5 shadow-sm sm:p-7 ${mlEvidence ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"}`}>
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex max-w-4xl items-start gap-3"><BrainCircuit className={`mt-0.5 h-7 w-7 shrink-0 ${mlEvidence ? "text-emerald-700" : "text-slate-500"}`} /><div><p className={`text-xs font-bold uppercase tracking-[0.16em] ${mlEvidence ? "text-emerald-800" : "text-slate-500"}`}>ML → Risk Twin · реальная связь</p><h2 id="ml-decision-title" className="mt-1 text-2xl font-bold tracking-tight text-slate-950">{mlEvidence ? "ML действительно изменил следующую задачу" : "Приоритет пока GIS-only: ML ещё не выполнен"}</h2><p className="mt-2 text-sm leading-6 text-slate-700">{mlEvidence ? integratedDecisionAction : "Запустите glacier-specific анализ: система создаст boundary, probability и entropy, затем пропустит их через quality gate перед включением в решение."}</p></div></div>
              {!mlEvidence && <button type="button" onClick={() => setForceMlRun(true)} disabled={contextStatus === "loading"} className="inline-flex min-h-11 items-center rounded-xl bg-emerald-700 px-4 text-sm font-bold text-white transition hover:bg-emerald-800 disabled:cursor-wait disabled:opacity-60">{contextStatus === "loading" ? "Анализируем…" : "Запустить ML для этого ледника"}</button>}
            </div>
            {mlEvidence && integratedCase && <>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <article className="rounded-2xl border border-emerald-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">ML boundary area</p><p className="mt-2 text-2xl font-black text-slate-950">{mlEvidence.metrics.predicted_area_km2.toFixed(4)} км²</p><p className="mt-1 text-xs text-slate-600">16‑канальный S2 + terrain + S1</p></article>
                <article className="rounded-2xl border border-emerald-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Raw ML ↔ RGI IoU</p><p className="mt-2 text-2xl font-black text-red-700">{(mlEvidence.metrics.rgi_overlap_iou * 100).toFixed(1)}%</p><p className="mt-1 text-xs text-slate-600">ниже gate 35% → обязательный review</p></article>
                <article className="rounded-2xl border border-emerald-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Uncertain review zone</p><p className="mt-2 text-2xl font-black text-slate-950">{(mlEvidence.metrics.uncertain_fraction_in_review_zone * 100).toFixed(1)}%</p><p className="mt-1 text-xs text-slate-600">predictive entropy, не event probability</p></article>
                <article className="rounded-2xl border border-emerald-100 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">ML review priority</p><p className="mt-2 text-2xl font-black text-emerald-800">{mlEvidence.metrics.review_priority_0_100}/100</p><p className="mt-1 text-xs text-slate-600">выше lake priority {integratedCase.decision.lake_observation_priority_0_100}</p></article>
              </div>
              <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-6 text-red-950"><strong>Критический честный вывод:</strong> raw ML показал −98,07% относительно старой RGI‑геометрии, но этот показатель заблокирован. Низкий IoU означает, что его нельзя называть таянием. ML полезен здесь тем, что автоматически перенёс первую задачу с общего осмотра озера на проверку ошибочной или спорной границы ледника.</div>
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-600"><p>Case <code>{mlEvidence.case_id}</code> · {mlEvidence.evidence_origin === "packaged_verified_snapshot" ? "проверенный локальный snapshot" : "локальный runtime inference"} · source crop SHA‑256 <code>{mlEvidence.source.source_crop_sha256.slice(0, 12)}…</code></p><Link href={`/ml?case=${encodeURIComponent(mlEvidence.case_id)}`} className="inline-flex min-h-10 items-center rounded-lg border border-emerald-700 px-3 font-bold text-emerald-900 hover:bg-white">Открыть ML‑доказательства</Link></div>
            </>}
          </section>

          <section aria-labelledby="action-title" className="rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm sm:p-7"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-6 w-6 shrink-0 text-amber-700" /><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-800">Практический результат</p><h2 id="action-title" className="mt-1 text-2xl font-bold tracking-tight text-amber-950">{integratedDecisionTitle ?? "Следующее действие: проверить контур воды"}</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-amber-950">{integratedDecisionAction ?? "Получить чистую спутниковую сцену и проверить контур воды."} Приоритет относится к сбору доказательств, а не к вероятности аварии.</p></div></div><ol className="mt-5 grid gap-3 md:grid-cols-3"><li className="rounded-xl bg-white/80 p-4"><span className="text-xs font-black text-amber-700">01</span><p className="mt-2 font-bold text-slate-950">Проверка ML‑границы</p><p className="mt-1 text-sm text-slate-600">Сверить raw и inventory-guided контуры с исходной сценой.</p></li><li className="rounded-xl bg-white/80 p-4"><span className="text-xs font-black text-amber-700">02</span><p className="mt-2 font-bold text-slate-950">Проверка озера</p><p className="mt-1 text-sm text-slate-600">Подтвердить водный контур на чистой сопоставимой сцене.</p></li><li className="rounded-xl bg-white/80 p-4"><span className="text-xs font-black text-amber-700">03</span><p className="mt-2 font-bold text-slate-950">Связь с оператором</p><p className="mt-1 text-sm text-slate-600">Сверить маршрут с инженерной схемой и только затем определять телеметрию.</p></li></ol></section>
          ...
          <section id="map" aria-labelledby="map-title" className="scroll-mt-20 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-cyan-800">Проверка на карте</p><h2 id="map-title" className="mt-1 text-2xl font-bold tracking-tight text-slate-950">ML‑граница → озеро → речной маршрут → объект компании</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Зелёный контур — реальный ML screening 2024; бирюзовый — RGI; синий — озеро; оранжевый — HydroRIVERS planning route; фиолетовый — объект компании. Геометрии остаются кликабельными и сохраняют источник.</p></div><MapPinned className="h-7 w-7 text-cyan-700" /></div><div className="mt-4"><RiskTwinMap glacier={selectedGlacier} objects={mapObjects} selectedObjectId={selectedObjectId} focusObjectId={selectedCase.lake_id ? `lake:${selectedCase.lake_id}` : `glacier:${selectedGlacier.rgi_id}`} onSelectObject={setSelectedObjectId} mode={companyAsset ? "route" : "evidence"} yearLayer={yearLayer} comparisonLayer={null} mlEvidence={mlEvidence} compact pinnedObjectIds={pinnedObjectIds} decisionContext={companyAsset ? { title: `Для ${companyAsset.name}`, instruction: integratedCase?.decision.driver === "ml_boundary_review" ? "1) проверить зелёную ML‑границу; 2) подтвердить контур озера; 3) сверить речную связь с инженерной схемой. Линия — не зона затопления." : "1) подтвердить контур озера; 2) сверить речную связь с инженерной схемой; 3) подключить телеметрию. Линия — не зона затопления.", metric: companyAssetDistanceToRouteM === null ? "Расстояние до маршрута вычисляется…" : `До оси HydroRIVERS: ${companyAssetDistanceToRouteM < 1000 ? `${companyAssetDistanceToRouteM.toFixed(0)} м` : `${(companyAssetDistanceToRouteM / 1000).toFixed(1)} км`}` } : null} /></div>{selectedObject && <p role="status" aria-live="polite" className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700"><strong>Выбрано на карте:</strong> {selectedObject.name}. {selectedObject.visibleFact}</p>}</section>

          <section id="details" aria-labelledby="details-title" className="scroll-mt-20 rounded-3xl border border-slate-200 bg-slate-100 p-5 shadow-sm sm:p-7"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-6 w-6 shrink-0 text-slate-700" /><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-600">Всё исследование сохранено</p><h2 id="details-title" className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Подробные доказательства — после основного решения</h2><p className="mt-2 text-sm leading-6 text-slate-600">Эти инструменты важны для проверки, но не нужны, чтобы понять главный результат за первые 30 секунд.</p></div></div><div className="mt-5 grid gap-3 md:grid-cols-3"><Link href={caseHref(selectedCase, mlEvidence?.case_id)} className="rounded-2xl bg-slate-950 p-5 text-white transition hover:bg-slate-800"><p className="text-xs font-bold uppercase tracking-wide text-cyan-200">Полный Risk Twin</p><p className="mt-2 font-bold">Все локальные слои, контекст маршрута и паспорт объекта.</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-cyan-200">Открыть <ChevronRight className="h-4 w-4" /></span></Link><Link href="/discovery" className="rounded-2xl bg-violet-700 p-5 text-white transition hover:bg-violet-600"><p className="text-xs font-bold uppercase tracking-wide text-violet-100">Похожие ледники</p><p className="mt-2 font-bold">100 сохранённых CryoGenesis‑паспортов и физически подобранные twin‑ледники.</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-violet-100">Открыть <ChevronRight className="h-4 w-4" /></span></Link><Link href="/analysis" className="rounded-2xl bg-white p-5 text-slate-950 transition hover:bg-slate-50"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Научная проверка</p><p className="mt-2 font-bold">ML‑метрики, uncertainty, строгий ряд и реестр утверждений.</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-cyan-800">Открыть <ChevronRight className="h-4 w-4" /></span></Link></div>{evidence && <details className="mt-4 rounded-2xl border border-slate-200 bg-white p-4"><summary className="cursor-pointer font-bold text-slate-950 focus-visible:outline-2 focus-visible:outline-cyan-700">Научные границы утверждений</summary><div className="mt-4 grid gap-3 md:grid-cols-2"><p className="rounded-xl bg-emerald-50 p-4 text-sm leading-6 text-emerald-950"><CheckCircle2 className="mr-2 inline h-4 w-4 text-emerald-700" />Локальный пакет: {evidence.release_checks.local_package_complete ? "готов" : "неполный"}; {evidence.release_checks.required_artifact_count} обязательных артефактов.</p><p className="rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-950"><AlertTriangle className="mr-2 inline h-4 w-4 text-amber-700" />Не заявляются: вероятность GLOF, официальное предупреждение, независимая внешняя точность или прогноз до 2050 года.</p></div></details>}</section>
        </>}
      </div>
    </main>
  </>;
