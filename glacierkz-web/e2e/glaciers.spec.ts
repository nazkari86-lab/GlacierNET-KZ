...
test.describe("Individual glacier registry", () => {
  ...
  test("shows Tuyuksu inventory evidence and reports physical-series availability honestly", async ({ page, request }) => {
    const seriesResponse = await request.get(
      `${API_URL}/api/glaciers/RGI2000-v7.0-G-13-33843/timeseries?method=ndsi`
    );
    expect(seriesResponse.ok()).toBeTruthy();
    const physicalSeries = await seriesResponse.json() as {
      points: Array<{ year: number; area_km2: number }>;
    ...
    if (physicalSeries.points.length >= 2) {
      await expect(page.getByText(`${physicalSeries.change_km2?.toFixed(4)} km²`)).toBeVisible();
      await expect(page.getByText(`${physicalSeries.change_percent?.toFixed(2)}%`)).toBeVisible();
    } else {
      await expect(page.getByText(/0 years/)).toBeVisible();
